...
Manages a 'failure' path node in an AWX workflow job template. A failure-path node will only execute when the parent node fails. This is useful for error handling and remediation workflows.
...
* `workflow_job_template_node_id` - (Required) The ID of the parent workflow node that this failure-path node follows.
